...
Lets the market-selector's browse trade type and its Market Info screen always reflect the currently selected trade type and market on the trade strip — never a previously selected one — across selector open/close cycles and trade-type switches, by resyncing the selector's local browse state to the live store `contract_type`.
