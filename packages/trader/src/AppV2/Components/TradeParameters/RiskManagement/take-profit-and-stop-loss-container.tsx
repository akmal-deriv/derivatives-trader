...
import TakeProfitAndStopLossInput, { TTakeProfitAndStopLossGate } from './take-profit-and-stop-loss-input';

type TTakeProfitAndStopLossContainerProps = {
    closeActionSheet: () => void;
    // Lifts the commit handler + state-backed dirty gate to the header owner (the picker), which
    // renders the single ActionSheet.Header check for the active tab.
    onActionsChange?: (actions: { onSave: () => void; is_save_disabled: boolean }) => void;
    should_show_deal_cancellation?: boolean;
};

const TakeProfitAndStopLossContainer = observer(
    ({ closeActionSheet, onActionsChange }: TTakeProfitAndStopLossContainerProps) => {
        const {
            has_take_profit,
            has_cancellation,
            has_stop_loss,
        ...
        const [tp_error_text, setTPErrorText] = React.useState<React.ReactNode>(
            validation_errors?.take_profit?.[0] ?? ''
        );
        const tp_ref = React.useRef({
            has_take_profit,
            take_profit,
            tp_error_text: validation_errors?.take_profit?.[0],
        });
        const is_api_response_tp_received_ref = React.useRef(false);

        const [sl_error_text, setSLErrorText] = React.useState<React.ReactNode>(
            validation_errors?.stop_loss?.[0] ?? ''
        );
        ...
        // Each input reports its state-backed dirty flag + blocking-error state; the header check enables
        // only when at least one field changed and neither field has a blocking error.
        const [tp_gate, setTPGate] = React.useState<TTakeProfitAndStopLossGate>();
        const [sl_gate, setSLGate] = React.useState<TTakeProfitAndStopLossGate>();

        const is_save_disabled =
            !(tp_gate?.is_dirty || sl_gate?.is_dirty) ||
            Boolean(tp_gate?.has_blocking_error) ||
            Boolean(sl_gate?.has_blocking_error);

        React.useEffect(() => {
            onActionsChange?.({ onSave, is_save_disabled });
            // eslint-disable-next-line react-hooks/exhaustive-deps
        }, [is_save_disabled, tp_gate, sl_gate]);
