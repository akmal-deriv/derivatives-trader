...
| Flow 5 | Resync after account switch (stays Running)           | ✅      | ✅     | `verify-automation-resync.spec.ts` — start run on real account (A) → Running → switch to demo (B) → switch back to A → run re-adopted as Running (no manual refresh) → Stop. Guards GRWT-9318 "stuck on Starting…". Second account = the demo account every login carries. 1/1 pass desktop + mobile. |
