...
> **Structural note:** Multipliers has no Duration param and no Barrier. The trade-page info panel shows **Stop out** (loss amount) and **Stop out level** (price) — Commission is not shown on the trade panel. Commission is rendered on the contract details page from the API: it is captured from the open contract details page and asserted exactly on the closed page. Stop out is captured pre-buy and asserted on the mobile closed Order Details row labelled "Stop out level" (which shows the amount).
...
| 6   | Capture pre-buy values            | `readStopOut()` — `stopOutValue` + assert `stopOutLevelValue` present                                   | `stopOut` captured; Stop out level row visible (value not captured)                                                                                                   | Both     |
...
| 13  | Open contract details (closed)    | `verifyClosedMultipliersContractDetailsPage()` — captures `sellId`                                      | Buy + Sell Ref. IDs; Multiplier, Stake, Commission (matches open), Stop out level (mobile, matches pre-buy `stopOut`); Start time, Entry spot (exact), Exit time/spot | Both     |
...
| 14  | Open contract details (closed)    | `verifyClosedMultipliersContractDetailsPage()` — captures `sellId`                                      | Buy + Sell Ref. IDs; Multiplier, Stake, Commission (matches open), Stop out level (mobile); Take profit (`30.01`), Stop loss (Not set); Start time, Entry spot, Exit time/spot | Both     |
...
| 12  | Close contract                    | `closeFirstContract()`                                                                                  | Contract card disappears from Positions                                                                                                                                        | Both     |
| 13  | Verify closed contract card       | `verifyClosedPositionsTab()` — captures `contractProfitLossAmount`                                      | Card shows market, trade type, stake, P&L                                                                                                                                      | Both     |
| 14  | Open contract details (closed)    | `verifyClosedMultipliersContractDetailsPage()` — captures `sellId`                                      | Buy + Sell Ref. IDs; Multiplier, Stake, Commission (matches open), Stop out level (mobile); Take profit (`21.32`), Stop loss (Not set); Start time, Entry spot, Exit time/spot | Both     |
| 15  | Verify balance after close        | `verifyBalanceAfterContractClose()`                                                                     | Balance = balanceBeforeClose + stake + P&L                                                                                                                                     | Both     |
| 16  | Reports — Trade table             | `verifyClosedContractInReports()` step 2                                                                | Row by `buyId`; dates, stake, contract value, P&L correct                                                                                                                      | Both     |
| 17  | Reports — Statement               | `verifyClosedContractInReports()` step 3                                                                | Sell row (by `sellId`) + Buy row (by `buyId`); balances correct                                                                                                                | Both     |
...
| 12  | Close contract                    | `closeFirstContract()`                                                                                  | Contract card disappears from Positions                                                                                                                                        | Both     |
| 13  | Verify closed contract card       | `verifyClosedPositionsTab()` — captures `contractProfitLossAmount`                                      | Card shows market, trade type, stake, P&L                                                                                                                                      | Both     |
| 14  | Open contract details (closed)    | `verifyClosedMultipliersContractDetailsPage()` — captures `sellId`                                      | Buy + Sell Ref. IDs; Multiplier, Stake, Commission (matches open), Stop out level (mobile); Take profit (Not set), Stop loss (`21.10`); Start time, Entry spot, Exit time/spot | Both     |
| 15  | Verify balance after close        | `verifyBalanceAfterContractClose()`                                                                     | Balance = balanceBeforeClose + stake + P&L                                                                                                                                     | Both     |
| 16  | Reports — Trade table             | `verifyClosedContractInReports()` step 2                                                                | Row by `buyId`; dates, stake, contract value, P&L correct                                                                                                                      | Both     |
| 17  | Reports — Statement               | `verifyClosedContractInReports()` step 3                                                                | Sell row (by `sellId`) + Buy row (by `buyId`); balances correct                                                                                                                | Both     |
...
| 12  | Close contract                    | `closeFirstContract()`                                                                                  | Contract card disappears from Positions                                                                                                                                                        | Both     |
| 13  | Verify closed contract card       | `verifyClosedPositionsTab()` — captures `contractProfitLossAmount`                                      | Card shows market, trade type, stake, P&L                                                                                                                                                      | Both     |
| 14  | Open contract details (closed)    | `verifyClosedMultipliersContractDetailsPage()` — captures `sellId`                                      | Buy + Sell Ref. IDs; Multiplier, Stake, Commission (matches open), Stop out level (mobile); Take profit (Not set), Stop loss (`23.01`); Start time, Entry spot, Exit time/spot                 | Both     |
| 15  | Verify balance after close        | `verifyBalanceAfterContractClose()`                                                                     | Balance = balanceBeforeClose + stake + P&L                                                                                                                                                     | Both     |
| 16  | Reports — Trade table             | `verifyClosedContractInReports()` step 2                                                                | Row by `buyId`; dates, stake, contract value, P&L correct                                                                                                                                      | Both     |
| 17  | Reports — Statement               | `verifyClosedContractInReports()` step 3                                                                | Sell row (by `sellId`) + Buy row (by `buyId`); balances correct                                                                                                                                | Both     |
